...
public class ApustuaEginGUI extends JFrame {
	...
	private final JButton btnApustuaGehitu = new JButton(
			ResourceBundle.getBundle("Etiquetas").getString("ApustuaGehitu"));
				...
				try {
					...
					if (zenb > 0.0) {
						if (zenb >= maxMinBet) {
							...
							lblError.setVisible(true);
							lblError.setText(
									ResourceBundle.getBundle("Etiquetas").getString("ApustuaError4") + " " + maxMinBet);
						}
					} else {
						lblError.setVisible(true);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (new Date().compareTo(businessLogic.findEvent(q).getEventDate()) < 0) {
					for (Quote quo : quoteVec) {
						if (businessLogic.findQuestionFromQuote(quo).getQuestionNumber()
								.equals(businessLogic.findQuestionFromQuote(q).getQuestionNumber())) {
							comp = true;
						}
					}
					if (!comp) {
