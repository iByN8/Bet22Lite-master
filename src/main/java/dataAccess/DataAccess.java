...
public class DataAccess {
	...
	public void initializeDB() {
		...
		try {
			...
			Event ev23 = new Event(23, "Atlanta Hawks-Houston Rockets", UtilDate.newDate(year, month, 17), team23,
					team24);
			Event ev24 = new Event(24, "Miami Heat-Chicago Bulls", UtilDate.newDate(year, month, 17), team25, team26);
			Event ev25 = new Event(25, "Boston Celtics-Memphis Grizzlies", UtilDate.newDate(year, month, 1), team27,
					team28);
	...
	public ExtendedIterator getEvents(Date date) {
		...
		ExtendedIteratorEvents it=new ExtendedIteratorEvents(res);
		return it;
	}
	...
	public Quote storeQuote(String forecast, Double Quote, Question question) throws QuoteAlreadyExist {
		...
		if (q.doesQuoteExist(forecast))
			throw new QuoteAlreadyExist("Kuota existitzen da");

		db.getTransaction().begin();
		Quote quo = q.addQuote(Quote, forecast, q);
		db.persist(quo); // db.persist(q) not required when CascadeType.PERSIST is added in questions
							// property of Event class
							// @OneToMany(fetch=FetchType.EAGER, cascade=CascadeType.PERSIST)
		db.getTransaction().commit();
		return quo;
	...
	public Collection<Question> findQuestion(Event event) {
		TypedQuery<Question> Qquery = db.createQuery("SELECT q FROM Question q WHERE q.getEvent() =?1 ",
				Question.class);
	...
	public ApustuAnitza createApustuAnitza(Registered user, Double balioa, LinkedList<Quote> quote,Integer apustuBikoitzaGalarazi) {
		...
		updateDiruaCAA(user, balioa);

		return apustuAnitza;
	}

	public ApustuAnitza createNewApustuAnitzaCAA(Registered user, Double balioa) {
		db.getTransaction().begin();
	...
	public void associateQuotesWithApustuCAA(LinkedList<Quote> quote, ApustuAnitza apustuAnitza) {
		db.getTransaction().begin();
		for (Quote quo : quote) {
	...
	public void updateApustuKopiaCAA(Integer apustuBikoitzaGalarazi, ApustuAnitza apustuAnitza) {
	...
	public void updateDiruaCAA(Registered user, Double balioa) {
		db.getTransaction().begin();
	...
	public List<Apustua> findApustua(Registered u) {
		Registered user = (Registered) db.find(Registered.class, u.getUsername());
		TypedQuery<Apustua> Aquery = db.createQuery("SELECT a FROM Apustua a WHERE a.getUser().getUsername() =?1 ",
				Apustua.class);
	...
	public List<ApustuAnitza> findApustuAnitza(Registered u) {
		Registered user = (Registered) db.find(Registered.class, u.getUsername());
		TypedQuery<ApustuAnitza> Aquery = db.createQuery(
				"SELECT aa FROM ApustuAnitza aa WHERE aa.getUser().getUsername() =?1 ", ApustuAnitza.class);
		Aquery.setParameter(1, u.getUsername());
		return Aquery.getResultList();
	}
	...
	public List<Transaction> findTransakzioak(Registered u) {
		Registered user = (Registered) db.find(Registered.class, u.getUsername());
		TypedQuery<Transaction> Tquery = db.createQuery(
				"SELECT t FROM Transaction t WHERE t.getErabiltzailea().getUsername() =?1 ", Transaction.class);
	...
	public void EmaitzakIpini(Quote quote, Date currentDate) throws EventNotFinished {
		...
		if (currentDate.compareTo(q.getQuestion().getEvent().getEventDate()) < 0) {
	...
	public List<Event> getEventsTeam(Team team) {
		...
		TypedQuery<Event> query = db.createQuery(
				"SELECT ev FROM Event ev WHERE ev.getLokala().getIzena()=?1 OR ev.getKanpokoa().getIzena()=?2",
				Event.class);
